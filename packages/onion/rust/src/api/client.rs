...
use super::session::{TorSession, TorSessionInner};
...
impl TorService {
    ...
    pub async fn start(
        ...
        Self::start_with_transport(state_dir, cache_dir, socks_port, TorTransport::Direct, None)
            .await
    }
    ...
    async fn start_with_transport(
        ...
        let default_session = TorSession::start(&client, socks_port).await?;
    ...
    pub async fn open_session(&self, socks_port: u16) -> Result<TorSession, TorFailure> {
        {
        ...
        let session = TorSession::start(&client, socks_port).await?;
...
mod tests {
    ...
    async fn sessions_use_distinct_clients_and_ports() {
        ...
        let svc = TorService::start(&format!("{dir}/s"), &format!("{dir}/c"), 0)
            .await
            .expect("start");
        let first = svc.open_session(0).await.expect("first session");
        let second = svc.open_session(0).await.expect("second session");
    ...
    async fn stopping_service_stops_every_session_and_rejects_new_ones() {
        ...
        let svc = TorService::start(&format!("{dir}/s"), &format!("{dir}/c"), 0)
            .await
            .expect("start");
        let session = svc.open_session(0).await.expect("session");

        svc.stop().await;

        assert!(!session.proxy_is_alive());
        let result = svc.open_session(0).await;
        assert!(
            matches!(
                result,
                Err(TorFailure {
    ...
    #[tokio::test]
    async fn proxy_rejects_clearnet_without_bootstrapping() {
